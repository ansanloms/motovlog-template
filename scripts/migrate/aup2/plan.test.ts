...
describe("planTimeline", () => {
  ...
  it("写真紹介の枠に置かれた動画は skip せず動画要素にする (拡張子は .mp4 に正規化、音は書かない)", () => {
    // FIXTURE の写真紹介の layer (layer=4) に 動画ファイル を 1 つ足す。
    // 原本の拡張子は .MOV (走行映像と同じく変換後の .mp4 に写る)。音は
    // 常に無音 (PhotoShowcase が volume: 0 で描く) なので、映像再生・
    // 音量フェード があっても写さない。
    const aup2 = parseAup2(
      [
        FIXTURE,
    ...
    expect(photoScenes).toContainEqual({
      ...
      at: 11.467,
      duration: 1.333,
      photos: [{ video: "V1.mp4", trimBefore: 3 }],
    });
