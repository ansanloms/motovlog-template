...
| ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `動画ファイル` (最も下の layer)                                     | `video()`。`再生位置` の開始秒が `trimBefore`、`フェード` が `fade()` の in/out、`映像再生` の `音量` と `音量フェード` が `volume` の折れ線。ファイル名は原本の拡張子を落とした `<basename>.mp4` (`npm run convert` の出力名)            |
...
| `動画ファイル` (写真紹介 layer)                                     | `photoShowcase()` の動画要素 (`再生位置` の開始秒が `trimBefore`)。音は常に無音のため `映像再生`・`音量フェード` は写さない。動画は `npm run convert` で `public/projects/<slug>/` に置いた変換済み素材を指す (写真の `photos/` ではない) |
| `音声ファイル` (それより上の layer) + 同じ開始フレームの `テキスト` | `narration()` の `line({ text, by })` (表情の切り替えは `by: { character, expression }`)                                                                                                                                                  |
