...
| `動画ファイル` (写真紹介 layer)                                     | `photoShowcase()` の動画要素 (`再生位置` の開始秒が `trimBefore`、`映像再生` の `音量` が `volume`)。動画は `npm run convert` で `public/projects/<slug>/` に置いた変換済み素材を指す (写真の `photos/` ではない)              |
