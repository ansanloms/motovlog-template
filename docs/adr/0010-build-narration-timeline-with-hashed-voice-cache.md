...
- 字幕の尺は `min(音声の実尺 + tail, 次の発話の開始 − 自分の開始)` とする (最後の発話は前者のまま)。
...
- 生成は `npm run dev` の watcher と `npm run render` の前段が行う。手で打つ生成コマンドは無い。
...
- `npm run dev` の watcher は timeline.ts の変更だけを監視する。起動中に theme の `narrator` (import 先のモジュール) を書き換えても、Node の ESM モジュールキャッシュにより watcher の再生成には反映されず、`npm run dev` の再起動が要る。
...
- 2026-09-09: 暗がりの保持 (5 秒) を止めて閾値を silenceGap に変更。理由: timeline は先まで分かっており、次が来ないと分かっている無音で保持する意味が無い。ちらつき対策の minOff は不要になった
