...
refs: [1, 2]
tags: [remotion, ffmpeg, proxy]
---

# ADR-0003: ドラレコ原本を H.264 のプロキシに変換して Remotion に読ませる
...
- H.264・30fps のプロキシを ext4 に置くと、静止画 1 枚が 4〜6 秒、10 秒のレンダリングが 54〜60 秒、深い位置からの 60 秒の持続レンダリングが 0.224 秒/フレームになる。`<OffthreadVideo>` へのフォールバックが起きていないことも確認した。
- シンボリックリンクで `public/` 配下に置いたファイルは、Remotion の配信サーバが拒否して 404 になる。
- 原本のプロキシ変換は、NVENC で 8 分 48 秒・出力 4.80GB (`-cq 23`)、libx264 では約 10.8 倍の時間がかかる。WSL では ffmpeg が `libcuda.so.1` を見つけられず、`LD_LIBRARY_PATH=/usr/lib/wsl/lib` の指定で解決した。
- Remotion Studio のプレビューでは HEVC 原本もプロキシも即時に追従し、差は出なかった。ただし編集とレンダリングで参照する `src` は同じファイルになる。
...
1. 原本をファイル単位で H.264 のプロキシに変換し、`public/projects/<slug>/` に実体で置く — 採用。`<Video>` がフォールバック無しで読め、静止画・レンダリングともに実用の速度になる。区間は Remotion 側の trim で切るので、編集の変更で再変換が起きない。
2. HEVC 原本を直接読む — 却下。静止画 1 枚に約 57 秒かかりタイムアウトする。`<Video>` が HEVC を扱えず `<OffthreadVideo>` にフォールバックする。
3. シンボリックリンクで原本やプロキシを `public/` 配下に見せる — 却下。配信サーバが拒否する。
4. 9P (`/mnt/c`) 上の実体を `--public-dir` で参照する — 却下。ext4 と同等という計測は直前の複製でキャッシュが温まった状態の小さな読み取りによるもので、一般化できない。プロキシは ext4 上で生成されるため、9P に置き直す利点が無い。
...
- composition の fps が原本 (24fps) と異なる場合、再標本化により原本とプロキシでフレームの対応が 1 対 1 にならない。
- 原本とプロキシの二重管理になり、プロキシは git 管理外なので復元には再変換が要る。
...
- `<OffthreadVideo>` へのフォールバックが起きる形式のファイルをプロキシとして置くこと。
