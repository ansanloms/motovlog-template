...
- 読みは `{漢字|よみ}` の記法で台本に書く。読みが無い部分はそのまま読む。字幕には漢字表記を、合成には読みを使う。この展開はスクリプトが行う。繰り返し使う固有名詞は ENGINE のユーザ辞書に登録してもよい。記法の片側が空 (`{|よみ}`・`{漢字|}`・`{|}`)、`|` が無い、入れ子や非対称の括弧 (閉じ忘れ・開き忘れ)、のいずれかであれば throw する。
- `line()` は合成に渡す文を `reading` として別に持てる (省略時は `text` をそのまま合成に使う)。`text` はそのまま字幕に出る漢字表記、`reading` は VOICEVOX に渡す文で、どちらも `{漢字|よみ}` 記法を書ける。
