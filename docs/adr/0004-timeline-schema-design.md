---
status: accepted
date: 2026-09-06T12:59:51Z
refs: [2]
tags: [remotion, timeline, schema]
---

# ADR-0004: timeline 定義を秒単位の JSON と zod schema で表現する

## Context

[ADR-0002](./0002-project-directory-layout.md) で、動画 1 本の定義 (timeline) を `projects/<slug>/timeline.json` に置き、Remotion には `--props` で渡し、`calculateMetadata` で schema の検証と尺の算出をすると決めた。timeline の中身の形式はまだ決めていない。
...
6. 互換性を切る変更を後から安全に扱えること

## Considered Options

1. 時間は秒単位、clips は順序リストで位置を導出し、他のトラックは絶対時刻で置く。zod schema を契約とし、`version` を持たせる — 採用。人が書く値は秒で、fps に依存しない。clips の重なりは `crossfadeIn` の上限を schema が検証するため矛盾を書けない。セリフや差し込みは映像と独立に絶対時刻で置ける。
...
5. zod schema を持たず TypeScript の型だけにする — 却下。JSON 入力の検証と既定値の補完ができず、`--props` で省略した項目が `undefined` のまま計算に渡る。Studio の編集 UI にも使えない。

## Decision

- timeline は 1 つの JSON オブジェクトとし、zod の `timelineSchema` を契約にする。`<Composition>` の `schema` に渡し、`calculateMetadata` で parse して既定値の補完と検証をする。parse を通っていない props をコンポーネントに渡さない。
...
- フィールドの追加は optional か既定値付きで行い、既存の timeline.json が `version` を変えずに parse を通るようにする。
...
- 既存の timeline.json が parse を通らなくなるフィールド変更を、`version` を上げずに行うこと。
...
| Remotion Studio でも `--props` で project を切り替えてプレビューできる          | 検証済 | 公式ドキュメント (get-input-props) に `remotion studio` と `remotion render` の両方に `--props` を渡せると明記 (2026-09-06)。ただし props-resolution は Studio への input props を推奨しないと注記し、input props は defaultProps と浅くマージされる。エンジンは input props があればそれだけを parse して混在を避ける |
| clips の順序リストが、実際の編集作業で絶対時刻より扱いやすい                    | 未検証 | 数本作った後に、クリップの挿入・削除でセリフとのずれをどう直したかを振り返る                                                                                                                                                                                                                                           |

## References

- [ADR-0002](./0002-project-directory-layout.md): timeline の置き場と `--props` での受け渡し、素材パスの規約。
