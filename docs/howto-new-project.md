...
- 立ち絵: 既存のキャラクターを使うなら `characters/<name>.ts` を import するだけでよい。新しいキャラクターなら `characters/<name>.ts` を書き (既存のものをコピーして直すのが早い)、PNG を `public/assets/characters/<name>/` に置く。`figure()` で layer に置く書き方は README の「立ち絵」参照。
