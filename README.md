...
Remotion でモトブログ動画を作るためのエンジン。動画 1 本 = 1 project とし、timeline の定義 (JSON) と素材からレンダリングする。設計上の決定は `docs/adr/` にある。

## 前提

- Node.js と npm (`npm ci` で依存を入れる)
- ffmpeg (プロキシ変換。NVENC を使う場合は NVIDIA GPU。WSL では `/usr/lib/wsl/lib` のライブラリを使う)
- jq (timeline.json の読み取り)
...
`--props` で渡した timeline.json だけが定義として使われ、サンプルの値とは混ざらない (Remotion は `--props` を `defaultProps` と浅くマージするため、コンテナを丸ごと省略した場合でもサンプルの値を引き継がない)。
