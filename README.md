...
字幕・暗がり等の見た目は `docs/design/tone-and-manner.md` (ADR-0007) で固定し、`src/theme.ts` の定数から読む。timeline では変えない (ADR-0008)。下部の暗がりは `lines` から自動で出す (語り出しの 0.3 秒前にフェードイン、無音が 5 秒続くとフェードアウト)。
