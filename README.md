...
- フレームレートは `projects/<slug>/timeline.ts` の `meta.fps` に合わせる。GOP 長は fps と同じ (1 秒ごとにキーフレーム)。`meta.fps` の読み取りに `npx tsx scripts/timeline-fps.ts <slug>` を使う。timeline.ts が無い・読めない場合はフォールバックせずエラーで止まる。
