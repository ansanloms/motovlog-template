...
describe("timeline", () => {
  ...
  it("layers が空なら throw する", () => {
    expect(() => timeline({ fps: 30 }, [])).toThrow();
  });
  ...
  it("at と after を同時に指定すると throw する (cut)", () => {
    ...
    expect(() => timeline({ fps: 30 }, [[item]])).toThrow();
  });
  ...
  it("duration が NaN / 0 / 負なら throw する (cut)", () => {
    expect(() =>
      timeline({ fps: 30 }, [[cut(null, { duration: Number.NaN })]]),
    ).toThrow();
    expect(() =>
      timeline({ fps: 30 }, [[cut(null, { duration: 0 })]]),
    ).toThrow();
    expect(() =>
      timeline({ fps: 30 }, [[cut(null, { duration: -1 })]]),
    ).toThrow();
  });

  it("duration が NaN / 0 / 負なら throw する (fade)", () => {
    expect(() =>
      timeline({ fps: 30 }, [[fade(null, { duration: Number.NaN })]]),
    ).toThrow();
    expect(() =>
      timeline({ fps: 30 }, [[fade(null, { duration: 0 })]]),
    ).toThrow();
    expect(() =>
      timeline({ fps: 30 }, [[fade(null, { duration: -1 })]]),
    ).toThrow();
  });
  ...
  it("duration が 1 フレーム未満なら throw する", () => {
    expect(() =>
      timeline({ fps: 30 }, [[cut(null, { duration: 0.01 })]]),
    ).toThrowError(/1 フレームに満たません/);
  });

  it("半フレームの item は位置によっては 1 フレームに満たず throw する", () => {
    expect(() =>
      timeline({ fps: 30 }, [[cut(null, { duration: 1 / 60, at: 0.02 })]]),
    ).toThrowError(/1 フレームに満たません/);
  });

  it("同じ duration: 1/60 でも at: 0 なら throw しない", () => {
    expect(() =>
      timeline({ fps: 30 }, [[cut(null, { duration: 1 / 60, at: 0 })]]),
    ).not.toThrow();
  });

  it("fps が 0 なら throw する", () => {
    expect(() =>
      timeline({ fps: 0 }, [[cut(null, { duration: 1 })]]),
    ).toThrow();
  });

  it("fps が NaN なら throw する", () => {
    expect(() =>
      timeline({ fps: Number.NaN }, [[cut(null, { duration: 1 })]]),
    ).toThrow();
  });
});
