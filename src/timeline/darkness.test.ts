...
import { computeDarknessSpans } from "./darkness";

const opts = { leadIn: 0.3, silenceGap: 5, fadeOut: 0.5 };

describe("computeDarknessSpans", () => {
  it("空なら空配列を返す", () => {
    expect(computeDarknessSpans([], opts)).toEqual([]);
  });

  it("1 本の line から区間を導く", () => {
    const spans = computeDarknessSpans([{ start: 1, duration: 2 }], opts);
  ...
  it("start がフェードイン秒未満なら 0 に clamp し、fadeIn も短縮される", () => {
    const spans = computeDarknessSpans([{ start: 0.1, duration: 1 }], opts);
  ...
  it("start が 0 の line では fadeIn も 0 になる", () => {
    const spans = computeDarknessSpans([{ start: 0, duration: 1 }], opts);
  ...
  it("次の line との隙間が閾値以内なら 1 区間に統合する", () => {
    ...
    const spans = computeDarknessSpans(
      [
        { start: 0, duration: 3 },
