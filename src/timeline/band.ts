...
// 掛かるのを避けるため。T&M の「無音 4.8 秒で消す」からのずれは最大
// fadeOut + leadIn (0.72 秒) に収まる。
//
// 例 (leadIn 0.24、silenceGap 4.8、fadeOut 0.48、閾値 5.52):
// - line1 が 1.0〜2.9 秒、line2 が 4.0〜7.96 秒 (サンプル project の値):
//   隙間は 1.1 秒で閾値以内なので 1 グループ。暗がりは 0.76 秒から出て
//   0.24 秒でフェードインし、7.96 + 4.8 + 0.48 = 13.24 秒まで続き、
//   最後の 0.48 秒でフェードアウトする。
// - line2 が 10.0 秒に始まる場合: 隙間 7.1 秒で閾値を超えるので 2 グループ。
//   暗がりは 2.9 + 5.28 = 8.18 秒でいったん消え、9.76 秒から出直す。
